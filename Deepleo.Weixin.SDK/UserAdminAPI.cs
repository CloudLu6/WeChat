...
namespace Deepleo.Weixin.SDK
{
    ...
    public class UserAdminAPI
    {
        ...
        public static dynamic GetInfo(string access_token, string code)
        {
            var client = new HttpClient();
            var result = client.GetAsync(string.Format("https://qyapi.weixin.qq.com/cgi-bin/user/getuserinfo?access_token={0}&code={1}", access_token, code)).Result;
            if (!result.IsSuccessStatusCode) return null;
            return DynamicJson.Parse(result.Content.ReadAsStringAsync().Result);
        }
