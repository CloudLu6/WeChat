...
namespace Deepleo.Weixin.SDK
{
    ...
    public class OAuth2API
    {
        ...
        /// </returns>
        public static dynamic GetUserInfo(string accessToken, string code, string agentId)
        {
            var client = new HttpClient();
            var result = client.GetAsync(string.Format("https://qyapi.weixin.qq.com/cgi-bin/user/getuserinfo?access_token={0}&code={1}&agentid={2}", accessToken, code, agentId)).Result;
            if (!result.IsSuccessStatusCode) return null;
            return DynamicJson.Parse(result.Content.ReadAsStringAsync().Result);
        }
        ...
        /// <returns></returns>
        public static dynamic GetUserId(string accessToken, string code)
        {
            var client = new HttpClient();
            var result = client.GetAsync(string.Format("https://qyapi.weixin.qq.com/cgi-bin/user/getuserinfo?access_token={0}&code={1}", accessToken, code)).Result;
            //var result1 = result;
            if (!result.IsSuccessStatusCode) return null;
            return DynamicJson.Parse(result.Content.ReadAsStringAsync().Result);
